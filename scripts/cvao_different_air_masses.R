...
setwd("D:/Documents - Copy/Cape Verde/nox/nox_data_analysis")
...
setwd("D:/Documents - Copy/Cape Verde/nox/data_submission/downloaded_data/simone")
...
# Reading in historical NO (2006-2017) ------------------------------------

# #no from 2006 to 2010 from files james had previously submitted - assuming data has been ozone corrected
# setwd("D:/Documents - Copy/Cape Verde/nox/data_submission/downloaded_data")
# 
# #data wasn't in utc (I don't think), so timezone has been set to utc to avoid DST changes
# #flagged data not available, so only using good data (V3 = 0) and data below LoD (V3 =2)
# early_dat = read.table("cv-noxy_capeverde_20061001_60min_filtered.na",skip = 48) %>% 
#   mutate(date_uk = as.POSIXct(round(V1 / 0.041667) *3600,
#                               origin = "2006-01-01 00:00",
#                               tz = "Europe/London"),
#          date = with_tz(date_uk,"UTC"),
#          no_flag = case_when(V3 == 0 ~ 0,
#                              V3 == 2 ~ 0.147),
#          no_ppt = ifelse(no_flag == 0 | no_flag == 2,V2,NA_real_)) %>% 
#   tibble() %>% 
#   select(date,no_ppt,no_flag)
...
# Reading in data (no2 offset corrected) ----------------------------------
...
nox = read.csv("output/data/cvao_nox_offset_ozone_corr.csv") %>% 
  ...
  bind_rows(ebas_no) %>% 
  arrange(date) %>% 
  mutate(no_flag = case_when(date >= "2012-04-12 13:00" & date <= "2012-05-10" ~ 0.456,
...
air_masses = read.csv("D:/Documents - Copy/Cape Verde/new_CVAO_sector_%_boxes_1.csv") %>% 
  rename(date = X) %>% 
  mutate(date = ymd_hms(date)) %>% 
  filter(date >= "2012-01-01",
         date < "2025-01-01") %>% 
...
png(filename = "D:/Documents - Copy/Writing/Thesis/Chapter 4 (NOx CVAO science)/Images/cheating/african_trend.png",
...
no_mean_airmass = dat_air_masses %>% 
  filter(date > "2017-01-01") %>%
...
dat_air_masses %>% 
  mutate(month = month(date),
         ...
         season = case_when(month >= 3 & month <= 5 ~ "Spring (MAM)",
                            ...
                            month >= 9 & month <= 11 ~ "Autumn (SON)",
                            TRUE ~ "Winter (DJF)")) %>% 
  # filter(air_masses == "North American/Atlantic") %>% 
...
dat_boxplot = dat_air_masses%>% 
  ...
  mutate(date = as.POSIXct.Date(date_only),
         ...
         air_masses = case_when(ocean >= 99.99 & south_atlantic < 1 | south_america < 1 & south_america > 0 & ocean >= 99.28 ~ "North Atlantic",
                                ...
                                europe > african & europe >= north_america & europe > 0  ~ "European/North Atlantic",
                                north_america > african & north_america > europe & north_america > 0 ~ "North American/Atlantic")) %>% 
         # air_masses = case_when(ocean >= 99.9 ~ "NAt",
         #                        # southern_hemisphere > 10 & polluted < 1 ~ "SH",
         #                        europe <= african & north_america <= african & african > 1 ~ "Afr (continent)",
         #                        europe > african & europe >= north_america & europe > 0  ~ "Eu/NAt",
         #                        north_america > african & north_america > europe & north_america > 0 ~ "Am/NAt",
         #                        african > 0 & african <= 1 & north_america <= african & europe <= african ~ "Afr (coast)")) %>% 
  select(date,daytime_no,daytime_no2,delta_ozone,upwelling:air_masses)
